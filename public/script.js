...
window.handleGoogleLogin = function () {
  if (!window.auth || !window.authSignInGoogle || !window.googleProvider) {
    if (typeof showToast === "function") {
      showToast("Layanan Google Auth sedang disiapkan. Tunggu sebentar...", "warning");
    } else {
      alert("Layanan Google Auth sedang disiapkan. Tunggu sebentar...");
    }
    return;
  }
  window
    ...
    .then((res) => {
      const name = res.user.displayName || res.user.email || "Pengguna";
      if (typeof showToast === "function") {
    ...
    .catch((e) => {
      ...
      if (e.code === "auth/popup-closed-by-user") {
        msg = "Login Google dibatalkan (popup ditutup).";
      } else if (e.code === "auth/unauthorized-domain") {
        msg = "Domain ini belum diizinkan di Firebase Console. Tambahkan domain ini di Firebase Auth > Settings > Authorized Domains.";
      } else if (e.code === "auth/popup-blocked") {
        msg = "Popup login diblokir browser. Izinkan pop-up di browser Anda.";
      } else if (e.message) {
        msg = `Login Google gagal: ${e.message}`;
      }
      if (typeof showToast === "function") {
        showToast(msg, "error");
      } else {
