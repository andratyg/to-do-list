  ...
  umum: {
    Senin: [
      ...
      },
      {
        ...
        guru: "Amanda Putri S, S.Pd",
        time: "13.20 - 14.40",
        type: "umum",
      },
      {
        mapel: "Bahasa Inggris",
        guru: "Endang Setiawan, S.Pd",
        time: "14.40 - 16.00",
        type: "umum",
      },
    ],
  ...
  setTimeout(() => {
    ...
    window.authListener(window.auth, (user) => {
      if (user) {
        ...
        initApp(uid); // Pass UID for Streak Check
      } else {
        currentUser = null;
...
function initApp(uid) {
  ...
  loadRandomQuote();
  updateTimerDisplay();
  injectNewUI(); // FITUR BARU: Inject HTML Music & Stats
  checkStreak(uid); // FITUR BARU: E
...
// --- FITUR BARU: UI INJECTION (Music, XP, Stats) ---
function injectNewUI() {
  ...
  // 3. Focus Stats Chart di Pomodoro Card
...
// --- FITUR BARU A: GAMIFICATION (XP & LEVEL) ---
function addXP(amount) {
  let stats = cachedData.gamification;
  stats.xp += amount;

  // Level Up Formula: Setiap 100 XP * Level saat ini
  ...
  if (stats.xp >= xpNeeded) {
    ...
    playSuccessSound("bell"); // Suara perayaan

    // Modal Level Up Sederhana
    alert(
...
function updateGamificationUI() {
  ...
  if (lvlBadge)
    lvlBadge.innerText = `Lvl ${stats.level} ${getLevelTitle(stats.level)}`;
}
...
function checkStreak(uid) {
  ...
  if (streak.lastLogin !== today) {
    ...
    if (streak.lastLogin === yesterdayStr) {
      ...
      streak.count = 1; // Reset jika terlewat sehari
    }
...
function renderFocusChart() {
  ...
  for (let i = 6; i >= 0; i--) {
    ...
    const dayName = days[d.getDay()].substring(0, 3); // Sen, Sel, ...
    const minutes = cachedData.focusLogs[dateStr] || 0;

    // Max bar height referensi 4 jam (240 menit)
    ...
    chart.innerHTML += `
            <div style="flex:1; display:flex; flex-direction:column; align-items:center; justify-content:flex-end; height:100%;">
                <div style="width:100%; background:rgba(255,255,255,0.3); height:${heightPct}%; border-radius:4px; position:relative; min-height: ${
      minutes > 0 ? 4 : 0
...
function renderAll() {
  ...
  loadTarget();
  loadPomodoroTasks();
  updateGamificationUI(); // Update UI baru
  renderFocusChart(); // Update UI baru
}

// --- FUNGSI KONTROL FOKUS (UPDATED FOR LOGGING) ---
function setFocusLock(lock) {
  ...
  if (focusModeElement) {
    ...
    document.querySelector(".timer-controls").style.marginTop = lock
      ? "10px"
      : "0";
  }
}
function handleTabBlur() {
  if (isFocusLocked && !isPaused && isWorking) {
    ...
    blurCount++;
    pauseTimer();
    showToast(`❌ JANGAN BANDEL! Fokus dijeda.`, "error");
  }
}
...
function handleBeforeUnload(event) {
  if (isFocusLocked && !isPaused && isWorking) {
    ...
    event.returnValue = "Mode Fokus aktif!";
    return "Mode Fokus aktif!";
  }
}
...
function playSuccessSound(type = "ding") {
  ...
  if (type === "ding") {
    ...
    g.gain.setValueAtTime(0.1, audioCtx.currentTime);
    g.gain.linearRampToValueAtTime(0.0001, audioCtx.currentTime + 0.3);
    o.start();
...
function toggleMode() {
  ...
  if (isWorking) {
    // Selesai Fokus -> Pindah Istirahat
    // Log Full Session
    const durationSetting = isExamMode
      ? WORK_DURATION_EXAM
      : WORK_DURATION_DEFAULT;
    ...
    addXP(20); // Bonus XP Selesai Pomodoro
    ...
    // Selesai Istirahat -> Pindah Fokus
    isWorking = true;
    savedFocusTime = null;
    timeLeft = isExamMode ? WORK_DURATION_EXAM : WORK_DURATION_DEFAULT;
...
function renderSchedule() {
  ...
  // Status Widget logic (maintained)
  let statusWidget = document.getElementById("liveStatusWidget");
  if (!statusWidget) {
    document.querySelector(
      ".schedule-status-bar"
    ).innerHTML = `<div id="liveStatusWidget" class="live-status-widget"><div class="status-icon-box"><i class="fas fa-bolt" id="statusIcon"></i></div><div class="status-content"><h4 id="statusLabel">STATUS SAAT INI</h4><p id="statusText">Memuat...</p></div></div>`;
  ...
  data.forEach((item, idx) => {
    ...
    // FITUR C: REAL NOTE (Gunakan ID unik: Hari_Index)
...
// --- FITUR BARU C: CATATAN MAPEL LOGIC ---
function openMapelNote(day, idx) {
  ...
  document.getElementById("noteModalTitle").innerText = `📝 Catatan: ${
    jadwalData[
...
// Override fungsi lama saveNoteFromModal
window.saveNoteFromModal = function () {
  ...
  closeNoteModal();
  renderSchedule(); // Refresh button style
  showToast("Catatan Mapel Disimpan!", "success");
};

// Override fungsi deleteNote
...
// --- TO-DO LIST (UPDATED WITH DRAG & DROP F) ---
...
function loadTasks() {
  ...
  // Sort: Belum selesai dulu, lalu tanggal
  filtered.sort((a, b) => {
  ...
  filtered.forEach((t, index) => {
    ...
    li.innerHTML = `
                <div class="task-content" style="display:flex;align-items:center;width:100%;">
                    <div class="check-btn" onclick="toggleTask(${
                      t.id
                    })"><i class="fas fa-check"></i></div>
                    <div class="task-text">
                        <span>${escapeHtml(t.text)}</span>
                        <small class="${badgeClass}">${dateDisplay} • ${
      t.priority
    }</small>
                    </div>
                ...
                <div class="task-actions">
                    <button class="action-btn" onclick="loadTaskToEdit(${
                      t.id
...
window.exportFinanceReport = function () {
  ...
  let csv =
    "Tanggal,Ket,Kat,Tipe,Jml,Dompet\n" +
    txns
      .map(
        (t) =>
          `${t.date},"${t.desc}",${t.category},${t.type},${t.amount},${t.wallet}`
      )
      .join("\n");
...
function renderExpenseChart(txns) {
  ...
  Object.keys(cats).forEach((c) => {
    ...
    html += `<div class="expense-item"><div class="expense-label"><span class="dot" style="background:${
      colors[c] || "#ccc"
    }"></span>${c}</div><div class="expense-value">Rp ${cats[c].toLocaleString(
      "id-ID"
    )} <small>(${pct}%)</small></div><div class="expense-bar-bg"><div class="expense-bar-fill" style="width:${pct}%;background:${
      colors[c] || "#ccc"
    }"></div></div></div>`;
  });
